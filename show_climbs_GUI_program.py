...
@author: jbrown888
10/7/24
Creates GUI that allows user to view climbs based on criteria
"""
...
class MultiFrameRoot(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        **kwargs: Arbitrary keyword arguments. These arguments are passed to the parent class initializer.
        ...
        self.fpath = os.path.join(dirpath, fname) # filepath to climbs data file

        self.show_frame("SelectionPage") # sets initial visible frame
        self.update()
    ...
    def recalculate_display_climbs(self):
        ...
        """        
        self.climbs_df = pd.read_csv(self.fpath, header=0,\
...
class DisplayPage(tk.Frame):
    ...
    def show_climbs(self):
        ...
        for k,v in self.controller.selection_dict.items():
            cc += f'{self.controller.frames["SelectionPage"].display_dict[(k, "labeltext")]} {", ".join(v)}\n'
